<?php
  ...
  if ($response['conexion']=='OK') { //Si la conexión es exitosa
    ...
    $email = "ariel@mail.com";
	  $nombre = "Ariel Peralta";
    $password = password_hash("ariel", PASSWORD_DEFAULT); //Encriptar la contraseña
    ...
    $password = password_hash("carlos", PASSWORD_DEFAULT); //Encriptar la contraseña
    ...
    $email = "juan@mail.com";
	  $nombre = "Juan Castro";
    $password = password_hash("juan", PASSWORD_DEFAULT); //Encriptar la contraseña
    $fecha_nacimiento = "1997-12-21";

    $insert->execute();
    $response['resultado']="1"; //Devolver resultado positivo
    $response['msg']= 'Ingresar Datos'; /*//Mostrar mensaje con la infirmación de los usuarios guardados
      ...
      $response['msg'] .='</br>contraseña: '.$password; //Mostrar la contraseña por defecto*/
